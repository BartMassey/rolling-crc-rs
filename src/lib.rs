...
/// next character.
#[inline(always)]
fn update_crc(crc: u32, crc_table: &CRCTable, c: u8) -> u32 {
...
fn make_crc_table(crc_table: &mut CRCTable, seed: u32) {
    let mut r = seed;
    crc_table[0] = 0;
    crc_table[128] = seed;
...
/// Make a rolling CRC table for the given window size.
/// This requires first computing the standard CRC table.
fn make_rolling_crc_table(winsize: usize,
                          crc_table: &CRCTable,
                          rolling_crc_table: &mut CRCTable)
{
    if INIT_CRC == 0 {
